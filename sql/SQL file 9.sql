...
-- 1. 강남 3구 사는 사람들을  조회(서초구,송파구,강남구) -  김철수 박영희 최영진
select * from customer where address in('서울시 서초구','서울시 송파구','서울시 강남구');

-- 2. email을 입력하지 않은 사람의 데이터를 출력하세요 - 이민수 (공백, 빈 문자열도 데이터임)
select * from customer where email is null;

-- 3. id가 5이상이며 주소가 서울시에 거주하는 사람 이름을 검색해라 - 강종현 서민지
--    단, 서울시는 다음과 같은 주소가 있다  강남구, 서초구, 송파구, 강서구 종로구
select * from customer where id >= 5 and address in('서울시 강남구','서울시 서초구','서울시 송파구','서울시 강서구','서울시 종로구');
-- select * from customer where id >= 5 and address like '서울시&'; " like, % "중요

-- 4. email이 NULL인 사람을 조회하는 쿼리 (IS NULL 을 사용해보세요) - 이민수
select * from customer where email is null;

-- 5. 이메일이 NULL 이 아니고 주문 번호가 2번이 넘으며 종로구에 사시는분은 - 서민지
select * from customer where email is not null and id >= 2 and address = '서울시 종로구';

-- 6. 주소가 '서울'이면서 id가 5이하인 고객 조회하는 쿼리 - 김철수 박영희 최영진
select * from customer where address like '서울시%' and id <= 5;

-- 7. 서울시에 사는 고객들을 조회하는 쿼리 (LIKE 사용해보기) - 김철수 박영희 최영진 강종현 서민지
select * from customer where address like '서울시%';

-- 8. 아이디가 3에서 8까지의 사람 중 '경기도 의정부'에 거주하고 있는 사람 중 이메일을 입력한 사람을 출력하시오 - 신영수
select * from customer where id between 3 and 8  and address = '경기도 의정부시' and email is not null;
